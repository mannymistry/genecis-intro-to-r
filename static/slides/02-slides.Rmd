...
factor | ordered, labelled variable | variable for strain of COVID19 labeled "Lambda", "Delta", etc.
...
For example, let's say I want to store the number of samples (a number), from reactor 1 ( a string), and whether this reactor is still operational (a logical) in a single object that preserves these different types... `r emo::ji("thinking")`
...
```{r}
reactor_1 <- list(1500, "Reactor1", TRUE)
...
Notice that `[[1]]`, `[[2]]`, and `[[3]]`, the element indices, have been replaced by the names `samples`, `reactor_name`, and `operational` `r emo::ji("eyes")`
...
Let's add another element to the list  regarding the qualitative observations of the smelliness of the reactor 

Notice use of `c()` to create the element `smell` `r emo::ji("eyes")`

```{r}
reactor_1 <- list(samples = 1500,
                reactor_name = "Reactor1",
                operational = TRUE,
                smell = c("sweet", "putrid"))
reactor_1
```
...
Now let's say 30% of all samples collected can't be used. How many samples from Reactor 1 potentiall cannont be used. Let's see if we can get that by multiplying the `samples` element by 0.30.
...
***
R is telling us that we supplied a non-numeric argument, i.e. `reactor_1`[1]`. 
...
***
Notice it no longer has the `$samples`. 
...
.panel[.panel-name[Example 4]
Now let's see how many samples can't be used
...
Let's save the unused samples transformation of the `samples` element as `unusable_samples`


```{r}
reactor_1$unusable_samples <- reactor_1$samples * 0.3
reactor_1
```
...
1. Create a list made up of data from your time at Genecis `year`, `name`, `title`, `love_about_has`,`love_r` . Make sure you enter two things you love about Has! Make your list object your name (Mine will be Mandip)
...
mandip <- list(year = 2021,
             name = "Mandip Mistry",
             title = "R Trainer", 
             love_about_has = c("funny", "loves a cuppa"), 
             love_r = TRUE)
mandip
```
]
...
As we saw with the object `smell` stored in the list `reactor_1`, objects within lists can have different dimensions and length. 

--

What if we wanted just one element of an object in a list, such as just the second element of `smell`? 

--

We can use indexing on the `smell` vector stored within the `reactor` list by chaining indexes.
...
```{r}
df_1 <- data.frame(time_point = c(1, 7),
                   mw = c(286,510), 
                   run = c("run1", "run1"))
...
```{r}
df_1$magic_recipe <- df_1$time_point + df_1$mw
...
.panel[.panel-name[Example 2]
Or we could replace an element using indexing too. Let's replace `time_point` with `time_point^2`:

```{r}
df_1$time_point <-  df_1$time_point^2
...
```{r}
df_1$time_point <- as.integer(df_1$time_point)
...
***
`*` We didn't talk about matrices today, you will learn more about them in the context of the General Linear Model
