...
```{r}
mandip <- list(4L, "Mandip Mistry", TRUE)
...
***
R is telling us that we supplied a non-numeric argument, i.e. `mandip[1]`. 
...
```{r}
mandip$dog_years <- mandip$year*7
mandip
```
...
As we saw with the object `fave_data` stored in the list `mandip`, objects within lists can have different dimensions and length. 
...
We can use indexing on the `fave_data` vector stored within the `mandip` list by chaining indexes.
